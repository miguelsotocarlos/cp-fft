...
// AC: https://codeforces.com/gym/103640/problem/B
#include <bits/stdc++.h>
using namespace std;
...
const ll mod = (1ull<<62)-(18ull<<32)+1;

vector<ll> compute(vector<ll> as, vector<ll> bs) {
	...
	if(b >= a) {
		...
		if(af[0] < 0) reverse(af.begin(), af.end());
		if(bf[0] < 0) reverse(bf.begin(), bf.end());
		for(ll &i : af) i = (i%mod+mod)%mod;
		for(ll &i : bf) i = (i%mod+mod)%mod;

		auto pol = conv_big(af, bf);
		for (ll &i : pol) if(i > mod/2) i -= mod;
		int d = b-a+1;
		forn(k, d) {
			res.push_back(total + pol[k]);
		}
	}
	return res;
}
...
int main(){
	...
	forn(i, n) cin >> as[i];
	forn(i, n) cin >> bs[i];
	auto mxs = compute(as, bs);
	forn(i, n) as[i] *=-1;
	auto mns = compute(as, bs);
	forn(i, n) cout << -mns[i] << " " << mxs[i] << "\n";
}

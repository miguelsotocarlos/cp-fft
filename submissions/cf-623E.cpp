...
// AC: https://codeforces.com/contest/623/problem/E
#include <bits/stdc++.h>
using namespace std;
...
Operator join(Operator a, Operator b) {
	int n = SZ(a.p);
	assert(SZ(b.p) == n);
	ll k = 1;
	forn(i, n) {
	...
	auto c = conv_sunzi(a.p, b.p, mod);
	while(SZ(c) > n) c.pop_back();
	return Operator { a.phi * b.phi % mod, c };
}

Operator pot(Operator a, int e) {
	int n = SZ(a.p);
	Operator res {1, vector<ll>(n, 0)};
	res.p[0] = 1;
	while(e) {
		if(e%2) res = join(res, a);
		a = join(a, a);
		e /= 2;
	}
	return res;
...
int main(){
	...
	if(n > k+10) {
		...
		vector<ll> facts(k+1, 1);
		forsn(i, 1, k+1) facts[i] = facts[i-1]*i%mod;

		vector<ll> invfacts(k+1);
		forn(i, k+1) invfacts[i] = po(facts[i], mod-2);

		vector<ll> p(k+1);
		forn(i, k+1) p[i] = invfacts[i];
		p[0] = 0;
		Operator r = pot(Operator{2, p}, n);
		ll res = 0;
		forn(i, k+1) res = (res+r.p[i]*invfacts[k-i])%mod;
		cout << (res*facts[k]%mod) << "\n";
	}
}
